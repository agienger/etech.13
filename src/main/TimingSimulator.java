...
public class TimingSimulator {
	 ...
	 * müssen mindestens einmal durch die Schaltung propagiert werden, bis sich
	 * ein stabiler Zustand einstellt. Um das festzustellen gibt es verschiedene
	 * Methoden (im Gatter mitzählen, wie oft sich der Wert ändert. Im Signal
	 * mitzählen, wie oft es geändert wurde). Bei diesem Propagieren darf
	 * natürlich nicht mit den Zeitverzögerungen gearbeitet werden. Sie können
	...
	/**
	 * Diese Methode führt die eigentliche Simulation durch. Dazu wird geprüft,
	 * ob in der EventQueue noch weitere Events vorhanden sind. Ist dies der
	 * Fall, dann wird das nächste anstehende Event behandelt. Dazu muss das
	 * Event die Methode propagate() zur Verfügung stellen, die dann das
	...
	public void simulate() {
		...
		while (queue.hasMore()) {
			...
			if (e.getSignal().getName().equals("S")
					|| e.getSignal().getName().equals("R")
					|| e.getSignal().getName().equals("Q")
					|| e.getSignal().getName().equals("nQ")) {
				String[] expectedResult = Solutions
						.getExpectedResultExcercise2().get(i);

				String[] actualResult = new String[] { e.getSignal().getName(),
						String.valueOf(e.getStartTime()),
						String.valueOf(e.getValue()) };
				for (int j = 0; j < 3; j++) {
					if (!expectedResult[j].equals(actualResult[j])) {
						System.out.println("FEHLER aufgetreten!");
						System.out.println("Erwartet: " + expectedResult[0]
								+ " Tatsaechlich: " + actualResult[0]);
						System.out.println("Erwartet: " + expectedResult[1]
								+ " Tatsaechlich: " + actualResult[1]);
						System.out.println("Erwartet: " + expectedResult[2]
								+ " Tatsaechlich: " + actualResult[2]);
						return;
					}
				}

				i++;
				if (i == Solutions.getExpectedResultExcercise2().size()) {
	 ...
	 * verstehen. Diese Methode wird benötigt, wenn Sie den Simulator ohne BlueJ
