...
/**
 * Klasse FullTimingSimulator ist ein Logiksimulator, der alle geforderten
 * Gattertypen benutzt und Zeitverzoegerungen beruecksichtigt. Die zu
 * simulierende Schaltung wird in der Methode <CODE>buildCircuitX()</CODE> der
 * Klasse erzeugt (mit X in 1,2,3). Diese Methode verwendet weitere
 * Hilfsmethoden um die Schaltung zu erzeugen. Versuchen Sie doch mal
 * herauszubekommen, was die Schaltung3 macht und bis zu welcher Taktfrequenz
 * sie zuverlaessig arbeitet. <BR>
 * Nach der Konstruktion der Schaltung muss zunaechst der Ruhezustand der
 * Schaltung berechnet werden. Dies �bernimmt die Methode
 * <CODE>findSteadyStateX()</CODE>. Die Eingabe-Events zur Stimulation der
 * Schaltung werden durch die Methode <CODE>setInputEventsX()</CODE> erzeugt.
 * Zum Testen Ihrer Klassen <CODE>Nand</CODE>, <CODE>Nor</CODE>, <CODE>Or</CODE>
 * , <CODE>And</CODE>, <CODE>Exor</CODE>, <CODE>Buf</CODE>, <CODE>Not</CODE>,
 * <CODE>FF</CODE>, <CODE>Latch</CODE>, <CODE>Signal</CODE>, <CODE>Event</CODE>
 * ,und <CODE>EventQueue</CODE> muessen Sie einfach nur eine Instanz dieser
 * Klasse erzeugen und dann die Methode <CODE>simulate()</CODE> aufrufen.
 * 
...
public class FullTimingSimulator {
	...
	private static List<String> calculatedSolutions;
	...
	// jeder Schaltung alle Eingaenge verwendet.
	...
	// Zaehler fuer die aufgebauten Multiplexer und Zaehler, um die Namen der
	// internen
	// Signale besser generieren zu k�nnen.
	private int muxCnt;
	private int cntCnt;

	public static void addCalculatedSolution(String solution) {
		calculatedSolutions.add(solution);
	}
	...
	public FullTimingSimulator(int version) {
		// Erzeugt die EventQueue fuer diesen Simulator
		queue = new EventQueue();
		calculatedSolutions = new ArrayList<String>();

		// Traegt diese EventQueue in ein statisches Feld der Klasse Event ein
	 ...
	 */
	private void buildMux(Signal[] inputs, Signal[] select, Signal[] nselect,
			Signal out) {
		muxCnt++;
	...
	/**
	 * Diese Methode konstruiert einen synchronen, ruecksetzbaren Zaehler.
	 * uebergeben werden ein Taktsignal, ein Reset-Signal und ein Array mit
	 * Augangssignalen. ueber die Größe dieses Arrays wird automatisch auch die
	 * Breite des Zählers bestimmt.
	 */
	...
	/**
	 * Diese Methode konstruiert eine weitere Schaltung. Simuliert wird ein
	 * einfacher vier Bit synchroner Zaehler.
	 */
	...
	/**
	 * Diese Methode konstruiert die dritte m�gliche Schaltung. Diese ist sehr
	 * kompliziert (enthaelt ca. 150 Gatter). Die genaue Funktion können Sie ja
	 * mal versuchen herauszubekommen.
	 */
	private void buildCircuit3() {
		...
		// Werden intern fuer den Aufbau der Multiplexer gebraucht
		...
		// Dazu werden zunaechst die beiden Haelften des Speichers getrennt
		...
		// Hier wird nun die Adressgenerierung fuer den Speicher aufgebaut.
		// Im wesentlichen handelt es sich hier um einen synchronen,
		// ruecksetzbaren Zaehhler, der immer dann zur�ckgesetzt wird, wenn
		// das oberste Bit des Speicherinhalts eine 1 liefert.
		buildSynCounter(clk, memOut[3], memOAddr);

		// Zaehlerausg�nge auch auf Signale fuehren, damit man die Adressierung
		// nachvollziehen kann.
		Buf[] obuf = new Buf[3];
		for (int i = 0; i < 3; i++) {
			obuf[i] = new Buf(1);
	 ...
	 * vern�nftige Initialwerte an die Eingaenge angelegt. Diese Initialwerte
	 * muessen mindestens einmal durch die Schaltung propagiert werden, bis sich
	 * ein stabiler Zustand einstellt. Um das festzustellen gibt es verschiedene
	 * Methoden (im Gatter mitzaehlen, wie oft sich der Wert aendert. Im Signal
	 * mitzaehlen, wie oft es geaendert wurde). Bei diesem Propagieren darf
	...
	/**
	 * Genau wie die vorhergehende Methode nur fuer die Schaltung zwei
	 */
	...
	/**
	 * Genau wie die vorhergehende Methode nur fuer die Schaltung drei
	 */
	...
	/**
	 * Ebenfalls die Erzeugung der Stimuli fuer die zweite eingebaute Schaltung
	 *
	 */
	...
	/**
	 * Ebenfalls die Erzeugung der Stimuli fuer die dritte eingebaute Schaltung
	 *
	 */
	...
	/**
	 * Main Methode dieser Klasse. Sie muessen das im Moment noch nicht
	 ...
	 * 
	 * Wenn Sie die verschiedenen Schaltungen testen wollen, dann muessen Sie
	 * den Parameter des Konstruktors entsprechend aendern. Das k�nnen Sie
	 * nat�rlich auch durch Auswertung der Kommandozeilenparameter tuen.
	 * 
	 * @throws FileNotFoundException
	 */
	static public void main(String[] args) throws FileNotFoundException {
		String fileNamePrefix = "src/main/resources/solutions/solutionfile";
		for (int testFall = 1; testFall <= 3; testFall++) {
			String fileName = fileNamePrefix + testFall;
			File solutionFile = new File(FullTimingSimulator.class.getResource(fileName).getFile());
